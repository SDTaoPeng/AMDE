...
namespace DifferentialEvolution
{
    public class DifferentialEvolution
    {
        ...
        public OptimizerOutput Optimizer(InputStructure inputstructurevar)
        {
            ...
            double a;
            double b;
            a = (double)Math.Round(((decimal)rd.Next(100, 200) / 200), 2);   
            ...
            while ((I_iter < I_itermax) && (S_bestval.FVr_oa[0] > F_thresholdmin))
            {
                ...
                int h;                                             
                Msize = NPinitial / 50;
                h = (int)Math.Round(((decimal)rd.Next(1, Msize)));                
                double Fi;
                double CRi;
                double c = (double)Math.Round(((decimal)rd.Next(0, 200) / 200), 2);   

                List<double> Sf = new List<double>();     
                List<double> Scr = new List<double>();   
                ...
                if (I_iter <= I_itermax / 2)
                {
                    ...
                    for (int i1 = 0; i1 < NPinitial;i1++ )
                    {
                        ...
                        for (i = 0; i < I_popnumber; i++)
                        {
                            for (j = 0; j < I_dimensionspara; j++)
                            {
                                FM_pm1[i, j] = FM_popold[FVr_a1[i], j];         //Mutation step求出的x
                        ...
                        double rand01;
                        rand01 = (double)Math.Round(((decimal)rd.Next(0, 200) / 200), 2);
                        if (I_strategy == 1)
                        {
                            for (i = 0; i < I_popnumber; i++)
                                for (j = 0; j < I_dimensionspara; j++)   
